    ...
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static">
        <Toolbar>
          ...
          {user.isAuth ?<><Button color="inherit">Profile</Button>
          <Button color="inherit" onClick={()=>{singOut()}}>Out Sign </Button>
          <IconButton aria-label="cart">
            <StyledBadge badgeContent={0} color="secondary">
              <ShoppingCartIcon />
            </StyledBadge>
          </IconButton></>
          :<Button color="inherit" onClick={()=>{loginClick()}}>{isLogin ? "Sing Up":"Log in"}</Button>}
        </Toolbar>
      </AppBar>
    </Box>
  );
